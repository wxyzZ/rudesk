...
class PlatformFFI {
  ...
  homedeskImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libhomedesk.so')
        : isLinux
            ? DynamicLibrary.open('libhomedesk.so')
            : isWindows
                ? DynamicLibrary.open('libhomedesk.dll')
                :
  ...
  void _startListenEvent(homedeskImpl homedeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = homedeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
